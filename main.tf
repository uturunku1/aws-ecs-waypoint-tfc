...
resource "aws_default_security_group" "tf_waypoint_inbound_internal" {
  vpc_id = aws_vpc.inbound_internal.id
  # name        = "example-nodejs-inbound-internal"
}

resource "aws_default_security_group" "tf_waypoint_inbound" {
  vpc_id = aws_vpc.inbound_internal.id
  # name        = "example-nodejs-inbound"
}

output "wp_security_inbound_internal" {
  value       = aws_default_security_group.tf_waypoint_inbound_internal.id
}
output "wp_security_inbound" {
  value       = aws_default_security_group.tf_waypoint_inbound.id
}
...
# terraform import aws_default_security_group.tf_waypoint_inbound_internal sg-0166caeecca88094f
# terraform import aws_default_security_group.tf_waypoint_inbound sg-0dc34cc1d070b7882
